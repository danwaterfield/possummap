...
# each tweet was stored on one line - here I was working with... god knows how many files
fname = 'oposs.json'
with open(fname, 'r') as f:
    
    #Create dictionary to later be stored as JSON. All data will be included
    # in the list 'data' This doesn't work in python 3.x thanks to dicts not being iterable
    users_with_geodata = {
        "data": []
    }
    all_users = []
...
# Save data to JSON file - the file name is replaceable with whatever you want
with open('oposs_geo.json', 'w') as fout:
